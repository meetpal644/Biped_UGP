...
class UpkieBalanceEnv(MujocoEnv):
    ...
    metadata = {
        ...
        "render_fps": 100,  # approx 1/(0.003*5)
    }
    ...
    def _get_reward(self, observation, torque):
        ...
        # r_velocity = -0.3 * (vx_com)**2
        
        # #pitch angle penalty
        # r_pitch = -20 * (pitch)**2
        # # Angular velocity damping
        # r_angular = -0.2 * np.sum(base_ang_vel**2)
        ...
        r_pitch   = - pitch**2
        r_zmp     = - zmp_error**2
        r_height  = - height_error**2
        r_velocity= - vx_com**2
        r_angular = - np.sum(base_ang_vel**2)
        r_torque  = - np.sum(torque**2)
        # -------------------------------------------------
        # Combined reward
        # -------------------------------------------------
        reward = (2 + 300.0 * r_pitch
        + 20.0  * r_height
        + 1.0   * r_velocity
        + 5.0   * r_angular
        + 1e-4  * r_torque)
